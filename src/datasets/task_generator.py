...
class Tasks_Generator:
    ...
    def generate_tasks(self):
        ...
        for i, (data, labels, _) in enumerate(warp_tqdm(self.loader, False)):
            # if len(data)<self.n_ways*self.shot+75:
        ...
        for key in tasks_dics[0].keys():
            ...
            if key == 'x_s' or key == 'x_q':
                # print(torch.cat([tasks_dics[i][key] for i in range(n_tasks)], dim=0).shape)
                merged_tasks[key] = torch.cat([tasks_dics[i][key] for i in range(n_tasks)], dim=0).contiguous().view(n_tasks,n_samples, 3, 84, 84)
            else:
                merged_tasks[key] = torch.cat([tasks_dics[i][key] for i in range(n_tasks)], dim=0).view(n_tasks,
                                                                                                        n_samples, -1)
        print(merged_tasks['x_s'].shape, merged_tasks['x_q'].shape)
