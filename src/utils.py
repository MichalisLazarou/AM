...
def extract_train_features(model, train_loader):
    model.eval()
    with torch.no_grad():
        base = []
        for i in range(10):
            base_loader_iter = iter(train_loader)
            x_base, y_base, _ = base_loader_iter.next()
            output_s, _ = model(x_base, feature=True)
            # print(output_q.device)
            base.append(output_s.cpu())
        base_fs = torch.cat(base, dim=0)
    return base_fs
...
def extract_mean_features(model, train_loader, args, logger, device):
    ...
    # filepath_mean = os.path.join(save_dir, 'output.plk')
...
def train_dim_reduction(model, train_loader, args, logger, device):
    """
        inputs:
            model : The loaded model containing the feature extractor
            train_loader : Train data loader
    ...
    # Load features from memory if previously saved ...
    save_dir = os.path.join(args.ckpt_path, args.model_tag, args.used_set)
    filepath_pca = os.path.join(save_dir, 'pca.sav')
    # filepath_mean = os.path.join(save_dir, 'output.plk')

    if not os.path.isfile(filepath_pca):

        logger.info(" ==> Beginning dimensionality reduction training")
        os.makedirs(save_dir, exist_ok=True)
        model.eval()
        with torch.no_grad():
            out_mean, fc_out_mean, ys = [], [], []
            for i, (inputs, labels, _) in enumerate(warp_tqdm(train_loader, False)):
                inputs = inputs.to(device)
                outputs, fc_outputs = model(inputs, True)
                # print(outputs.shape)
                out_mean.append(F.normalize(outputs, dim=1, p=2).cpu().data.numpy())
                ys.append(labels.cpu().data.numpy())
                if fc_outputs is not None:
                    fc_out_mean.append(fc_outputs.cpu().data.numpy())
            out_mean = np.concatenate(out_mean, axis=0)#.mean(0)
            ys = np.concatenate(ys, axis=0)#.mean(0)
            #------------------------training PCA-----------------------------------------------------------------------
            import plda.classifier
            pca = plda.classifier.Classifier()
            pca.fit_model(out_mean, ys, 24)
            # logger.info(" ==> Saving plda to {}".format(filepath_pca))
            ...
            # pca = KernelPCA(n_components=128)#Isomap(n_components=32, n_neighbors=30)
            # pca.fit(out_mean, ys)
            logger.info(" ==> Saving pca to {}".format(filepath_pca))
            pickle.dump(pca, open(filepath_pca, 'wb'))
            return pca
    else:
        clf = pickle.load(open(filepath_pca, 'rb'))
        logger.info(" ==> Features loaded from {}".format(filepath_pca))
        return clf
