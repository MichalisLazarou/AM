...
class PT_MAP(object):
    ...
    def run_task(self, task_dic, shot, reductor=None):
        ...
        datas = self.QRreduction(datas)
        datas = self.scaleEachUnitaryDatas(datas)
        datas = self.centerDatas(datas)
        # datas = self.centerData(datas, shot)
        # if self.PLC:
        #     # datas = self.PT(datas)
        #     datas = self.QRreduction(datas)
        #     datas = self.scaleEachUnitaryDatas(datas)
        ...
        #     datas = F.normalize(datas, dim=2)  # - train_mean.cuda()#
        #     if reductor != None:
        #         features = []
        #         for i in range(len(datas)):
        #             # print(datas.shape)
        #             z_all = torch.Tensor(reductor.fit_transform(datas[i].cpu())).cuda()
        #             # z_all = torch.Tensor(reductor.model.transform(datas[i].cpu(),from_space='D', to_space='U_model')).cuda()
        #         #     # print(z_all.shape)
        #         #     # output_s, output_q = z_all[:support.size(1)], z_all[support.size(1):]
        #         #     # print(output_q.shape, output_s.shape)
        #             features.append(z_all),
        #         datas = torch.stack(features).cuda()
                # query = torch.stack(fq).cuda()

        # Transfer tensors to GPU if needed
        data = datas.to(self.device)
